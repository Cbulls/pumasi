...
| 답변 검증 | 필수 여부 + 유형별 값 검증(`AnswerValidator`): 보기 소속, 선택 개수, 글자수/정규식, 척도 범위, 날짜·시간, 그리드 `행=열` |
...
응답형 11종: `SHORT_TEXT`, `LONG_TEXT`, `RADIO`, `CHECKBOX`, `DROPDOWN`, `LINEAR_SCALE`, `RATING`, `DATE`, `TIME`, `MULTIPLE_CHOICE_GRID`, `CHECKBOX_GRID`  
콘텐츠 블록(응답·비용 제외): `DESCRIPTION`, `IMAGE` · 첨부: `FILE`

### 그리드 (`MULTIPLE_CHOICE_GRID` / `CHECKBOX_GRID`)

- 행: `QuestionVO.rowLabels` → DB `form_question.row_labels` (JSONB, V15)
- 열: 기존 `form_question_option` (`options`)
- 답: `AnswerVO.values`에 `"행라벨=열라벨"` (스키마 변경 없음). 라벨에 `=` 금지
- MC 그리드: 필수 시 행마다 정확히 1개 · 체크 그리드: `(행,열)` 유일, `minSelect`/`maxSelect`는 **행당**
- 집계: `ResultAggregator` → `chartType=matrix`, counts 키 `"행=열"`. 결과 항목에 `rowLabels`·`columnLabels`
- 비용: 그리드 1문항 = 1분(다른 응답형과 동일)
...
```bash
gradle bootRun                # Flyway가 V1~V15 마이그레이션 + seed 자동 적용
```
...
- `SettlementCalcTest`, `QualityJudgeTest`, `AnswerValidatorTest`, `FormValidatorTest`, `ResultAggregatorTest`, `ImageAssetServiceTest`: DB 없이 도는 순수 로직 검증(DROPDOWN/RATING/DATE/TIME·그리드·주의 문항·matrix 집계 포함).
...
| V13 | user_account.email, magic_link_token, user_notification |
| V14 | form.confirmation_message, form_question.allow_other / shuffle_options |
| V15 | form_question.row_labels (객관식/체크박스 그리드 행 라벨) |

## 범위 밖 (현 시점)

실인증 고도화(매직링크는 V13 스키마·API 일부 존재, 데모 로그인 게이트와 병행), 실제 PG 연동(Fake 충전으로 대체),
POI xlsx(CSV로 대체), 하이브리드 랭킹 점수식 전체(현재는 상호 부스트+채움률 정렬),
퀴즈 채점·무로그인 응답, Redis 분산 rate limit, Sentry 등 에러 알림.
