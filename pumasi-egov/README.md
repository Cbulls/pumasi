...
| 결정 | 내용 |
|---|---|
| 인증 | 로그인 토큰(`Authorization: Bearer`)만 신뢰. 비밀번호 없는 계정 선택 로그인(데모 한정, 소셜 로그인 미구현). seed 유저로 시연 |
...
| 답변 검증 | 필수 여부 + 유형별 값 검증(`AnswerValidator`): 보기 소속, 선택 개수, 글자수/정규식, 척도 범위 |
...
```bash
gradle bootRun                # Flyway가 V1~V6 마이그레이션 + seed 자동 적용
```
...
- `SettlementCalcTest`, `QualityJudgeTest`, `AnswerValidatorTest`: DB 없이 도는 순수 로직 검증.
...
| GET | `/pmsi/feed` | 응답 피드(게시 중 + 남의 설문 + 미응답만) |
...
- 어뷰징: 소요시간 서버 측정(start→submit), pass 상한(maxResponses) 도달 시 자동 마감.
