 ...
 *
 * 호출 경로: 큐 컨슈머 → ingest(). 비동기성은 큐가 제공하므로 이 메서드는 동기.
 *   적재 실패 시 예외가 컨슈머→브로커로 전파되어 재시도/DLQ가 작동한다(유실 방지).
 */
@Service
public class EventIngestService {
    ...
    /**
     * 큐 컨슈머가 호출한다. @Async 아님 — 컨슈머 스레드에서 동기 실행되어야
     * 적재 실패 예외가 브로커로 전파되어 nack→재시도→DLQ가 작동한다(Q2/Q3).
     * (이전엔 컨트롤러가 @Async로 호출 → 실패가 조용히 유실됐다. 그 위험을 큐로 대체.)
     */
    @Transactional
    public void ingest(EventRequest req) {
        ...
        // 2-1) last_event_at 갱신. 멱등 무시(recordEvent=false)와 무관하게 호출 —
        //      활동은 실제로 있었으므로 타임아웃 판정에 반영돼야 한다(E5).
        //      occurredAt 누락/미래값 처리는 엔티티가 담당(E3/E4), 순서 역전 방어 MAX(E2).
        session.touchLastEvent(req.occurredAt(), java.time.Instant.now());
